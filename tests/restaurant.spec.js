...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  ...
  it('2 - retorna um objeto cujas chaves são somente `food` e `drink`', () => {
  ...
  it('3 - Verifique se o menu passado pra função createMenu() é idêntico ao menu recuperado pela função', () => {
  ...
  it('8 - Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`', () => {
    ...
    // ```
    const res = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    res.order('coxinha');
    expect(res.pay()).toBe(4.29);
